...
"        const catArr = Object.entries(catMap).map(([cat, count]) => ({ categoria: cat, totalProductos: count as number, totalUnidades: count as number }));",
...
"              <tbody>",
"                <tr><td>Normal (stock mayor a 5)</td><td className={styles.bold}>{stockData.normal || 0}</td></tr>",
"                <tr><td><Badge variant='warning'>Bajo (1-5 unidades)</Badge></td><td className={styles.bold}>{stockData.bajo || 0}</td></tr>",
"                <tr><td><Badge variant='error'>Agotado (sin stock)</Badge></td><td className={styles.bold}>{stockData.sinStock || 0}</td></tr>",
"              </tbody>",
"            </table>",
"          )}",
"        </div>",
...
"fs.writeFileSync('src/app/admin/reportes/page.tsx', code);",
"console.log('ok');",
